    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _map(
        ...
        for ordinal in range(out.size):
            ...
            to_index(ordinal, out_shape, out_index=out_idx)
            in_idx = np.zeros(len(in_shape), dtype=np.int32)
            broadcast_index(out_idx, out_shape, in_shape, out_index=in_idx)
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _zip(
        ...
        for ordinal in range(out.size):
            ...
            broadcast_index(out_idx, out_shape, a_shape, out_index=a_idx)
            b_idx = np.zeros(len(b_shape), dtype=np.int32)
            broadcast_index(out_idx, out_shape, b_shape, out_index=b_idx)
...
def tensor_reduce(fn: Callable[[float, float], float]) -> Any:
    ...
    * `out_shape` will be the same as `a_shape`
    ...
    def _reduce(
        ...
        was = set()
        for ordinal in range(a_storage.size):
            a_idx = np.zeros(len(a_shape), dtype=np.int32)
            to_index(ordinal, a_shape, out_index=a_idx)
            a_pos = index_to_position(a_idx, a_strides)
            a_idx[reduce_dim] = 0
            out_pos = index_to_position(a_idx, out_strides)
